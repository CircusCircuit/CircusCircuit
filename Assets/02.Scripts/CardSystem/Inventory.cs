...
namespace CardSystem
{
    public class Inventory : CardAdaptManager
    {
        ...
        [SerializeField] Sprite oneOfTwoCardImg;
        ...
        private void OnEnable()
        {
            ...
            CardArrange(SlotRowActivation());
        }

        int SlotRowActivation()
        {
            int cardNumb = TotalCardManager.mergeCardList.Count
                + TotalCardManager.normalCardList.Count
                + TotalCardManager.oneOfTwoCardList.Count;
        ...
        void CardArrange(int _numb)
        {
            if (_numb == 0) return;

            StartCoroutine(ArrayMerge(_numb));
        }

        IEnumerator ArrayMerge(int _numb)
        {
            if (TotalCardManager.mergeCardList != null)
            {
                for (int i = 0; i < TotalCardManager.mergeCardList.Count; i++)
                {
                    slot[i].GetComponent<Image>().sprite = mergeCardImg;
                    slot[i].SetActive(true);
                    activeSlotCount++;
                }
            }

            yield return StartCoroutine(ArrayMerge(_numb));

            StartCoroutine(ArrayNormal(_numb));
        }

        IEnumerator ArrayNormal(int _numb)
        {
            if (TotalCardManager.normalCardList != null)
            {
                int idx = 0;
                for (int i = activeSlotCount; i < activeSlotCount + TotalCardManager.normalCardList.Count; i++)
                {
                    slot[i].GetComponent<Image>().sprite = TotalCardManager.normalCardList[idx].normalInfo.cardImage;

                    print(i + "가 outOfIndex?");
                    print(idx + "가 outOfIndex?");
            ...
            yield return StartCoroutine(ArrayNormal(_numb));

            StartCoroutine(ArrayOneOfTwo(_numb));
        }

        IEnumerator ArrayOneOfTwo(int _numb)
        {
            if (TotalCardManager.oneOfTwoCardList != null)
            {
                for (int i = activeSlotCount; i < _numb; i++)
                {
                    slot[i].GetComponent<Image>().sprite = oneOfTwoCardImg;
                    slot[i].SetActive(true);
                    activeSlotCount++;
                }
            }

            yield return StartCoroutine(ArrayOneOfTwo(_numb));
        }
        ...
        void SlotReArrange()
        {
            ...
            print("mergeInfoList_Count" + mergeInfoList.Count);
